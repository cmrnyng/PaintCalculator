...
const validateInput = (input, checkInt) => {
...
const validateNum = (input) => validateInput(input, false);
// Validate integers
const validateInt = (input) => validateInput(input, true);
// Function to remove invalid inputs from the input field after the user submits
const filterInput = (input, checkInt) => {
...
const filterNum = (input) => filterInput(input, false);
// Remove any input which is not an integer
const filterInt = (input) => filterInput(input, true);
...
const initialQuestions = await inquirer.prompt([
    ...
    },
    {
        ...
        message: answers => `Hey ${answers.userName}, please enter the total number of walls you would like to paint:`,
        validate: (input) => validateInt(input) ? true : "Please enter a valid whole number greater than 0.",
        transformer: (input) => {
            const num = Number(input);
    ...
    const wallInfo = await inquirer.prompt([
        {
            ...
            validate: (input) => validateNum(input) ? true : "Please enter a valid number greater than 0.",
            transformer: (input) => (validateNum(input) ? input : chalk.red(input)),
        },
        {
            type: "input",
            name: "height",
    ...
    const exclusions = await inquirer.prompt([
        {
            ...
            validate: (input) => !validateNum(input)
                ? "Please enter a valid number greater than 0."
                : Number(input) >= Number(wallInfo.len)
                    ? `Please enter a number less than the length of the wall (${Number(wallInfo.len)}m).`
                    : true,
            transformer: (input) => validateNum(input) && Number(input) < Number(wallInfo.len) ? input : chalk.red(input),
        },
        {
            type: "input",
            name: "height",
            ...
            validate: (input) => !validateNum(input)
                ...
                : Number(input) >= Number(wallInfo.height)
                    ...
                    : true,
            transformer: (input) => validateNum(input) && Number(input) < Number(wallInfo.height) ? input : chalk.red(input),
        },
    ]);
    walls.push(new Wall(Number(wallInfo.len), Number(wallInfo.height), wallInfo.colour, Number(exclusions.len), Number(exclusions.height)));
}
